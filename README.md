...
このプロジェクトは、GitHub Actionsを使用した継続的インテグレーション（CI）を実装しています。

### CI ワークフロー

**メインワークフロー** (`.github/workflows/ci.yml`):
- ✅ コード品質チェック（フォーマット、Lint、セキュリティ監査）
- ✅ 単体テスト（126テスト）
- ✅ 統合テスト（27テスト、MySQL 8.0使用）
- ✅ マイグレーションチェック
- ✅ AIガードレールチェック（アーキテクチャ違反、セキュリティ）

**カバレッジワークフロー** (`.github/workflows/rust.yml`):
- ✅ 単体テストとコードカバレッジ測定
- ✅ Codecovへのレポート送信
...
1. **品質保証**: すべてのPRで自動的にテストとチェックを実行
2. **セキュリティ**: 脆弱性とセキュリティ問題の早期発見
3. **アーキテクチャ保護**: Clean Architectureの原則違反を検出
4. **リグレッション防止**: 既存機能の破壊を防止

### 詳細ドキュメント

CI環境の詳細については以下を参照してください：
- **[doc/CI_TESTING.md](doc/CI_TESTING.md)**: CI環境の詳細とトラブルシューティング
- **[doc/AI_GUARDRAILS.md](doc/AI_GUARDRAILS.md)**: AIガードレールの詳細
